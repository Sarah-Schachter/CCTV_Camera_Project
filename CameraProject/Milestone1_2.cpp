...
//using namespace cv;
//
//int detect_frames(std::queue<cv::Mat>& framesQueue);
//
//#pragma region Task 1 divide to frames
//queue<Mat> divideCaptureToFrames(string path) {
//	Mat frame;
...
//
//
//#pragma region Task 3 send to yolo
...
//	std::ifstream ifs(R"(.\config_files\classes.txt)");
//	std::string line;
...
//{
//	auto result = cv::dnn::readNet(R"(.\config_files\yolov5s.onnx)");
//	//auto result = cv::dnn::readNet(R"(C:\Users\user\Documents\Kamateck\camera_project\YOLO1\yolov5-opencv-cpp-python\config_files\yolov5s.onnx)");
//	if (is_cuda)
//	{
...
//	{
//		cv::Mat frame = framesQueue.front();
//		if (frame.empty())
//		{
//			cout << "End of stream\n";
//			break;
...
//
//
//vector <Detection>  detectOne(cv::Mat frame, vector <string> class_list, cv::dnn::Net net, std::chrono::steady_clock::time_point start, int frame_count, float fps, int total_frames)
//{
//	vector <Detection> output;
//
//	if (frame.empty())
...
//
//void calcAverageColor(const Mat& image, double& avgR, double& avgG, double& avgB)
//{
//	Scalar avgColor = mean(image);
...
//
//
//void saveToCSV(const string& filename, const Detection& detection, double avgR, double avgG, double avgB)
//{
//	ofstream file;
...
//}
//
//void calcSaveDetectoins(Mat currentFrame, vector <Detection> detections)
//{
//	for (const Detection& detection : detections) {
//		double avgRectR, avgRectG, avgRectB;
//		Mat detectionROI = currentFrame(detection.box); // get the small image of the detection area only
...
////int main()
////{
////	divideCaptureToFrames(R"(C:\WIN_20230907_14_08_38_Pro.mp4)");
////	return 0;
////
////
////
